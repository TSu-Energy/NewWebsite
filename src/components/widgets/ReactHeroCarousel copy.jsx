  ...
  return (
    <div className="relative">
      ...
      <div
        ...
        style={{ backgroundImage: `url(${activeButtonData.background})` }}
      ></div>

      {/* 主体内容 */}
      <main className="relative z-10 flex flex-col md:flex-row items-center justify-between p-6">
        {/* 左侧内容 */}
        <div className="left-hero space-y-4 max-w-md">
          <h1 className="text-2xl font-bold">我们使用领先 AI 技术</h1>
          <h2 className="text-lg">赋能全球供应链 ESG 建设和可持续发展</h2>

          {/* 按钮部分 */}
          <div className="relative z-10 flex flex-wrap gap-4 text-left p-6">
            {buttons.map((button) => (
              <button
                key={button.id}
              ...
              >
                {button.label}
                <img src={button.image} alt={`${button.label} 图标`} />
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-4">
          {/* 示例卡片 */}
          <div className="relative flex flex-wrap gap-10 z-20">
            {activeButtonData.rightContent.cards.map((card, index) => (
              <div key={index} className={`absolute ${card.position} ${card.width}`}>
          ...
          {/* 描述文案 */}
          <div className="bg-blue-900 text-white p-6 rounded shadow-md z-30">
            <p className="mb-4">{activeButtonData.rightContent.description}</p>
            <div className="flex justify-end">
              <button className="flex items-center gap-2 text-white border border-white px-4 py-2 rounded hover:bg-white hover:text-blue-900">
                <span>了解更多</span>
